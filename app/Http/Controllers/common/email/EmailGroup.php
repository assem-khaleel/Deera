...
class EmailGroup extends Controller
{
    ...
    public function create(Request $request,rUsers $rUsers)
    {
        ...
        $oUsers=mUsers::with('role')->where('type','=',config('array.users_type_registered_index'))->get();


        foreach($oUsers as $oUser){
            $usersList[$oUser->id]= $oUser->first_name;
            if(count($oUser->role)){
                foreach($oUser->role as $role){ $usersList[$oUser->id].='-'. $role->name.' ';}}
        }
