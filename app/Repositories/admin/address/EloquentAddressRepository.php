...
$aResults[$result->id]=$result->name;
    ...
    public function exportExcel($request){
        ...
        \Excel::create('Addresses List', function($excel) use($excelData) {
        ...
        if(count($oResults)){
            ...
            foreach($oResults as $oResult){
                $aResults[]=[
                    ...
                    $oResult->area->name_en,
                    $oResult->name,
                    (array_key_exists($oResult->type,config('array.address_type')))?config('array.address_type')[$oResult->type]:'',
                    $oResult->mobile,
                    (array_key_exists($oResult->notification,config('array.address_notification')))?config('array.address_notification')[$oResult->notification]:'' ,
                    $oResult->created_at
                ];
            }
        }
