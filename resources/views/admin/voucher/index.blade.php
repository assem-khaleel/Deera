            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="white-box">
                        ...
                        @if($canCreate)
                        <div class="col-xs-3">
                            <a  href="{{route('admin.voucher.create')}}"class="btn btn-primary form-control">
                        ...
                        <table class="tablesaw table-bordered table-hover table" data-tablesaw-mode="swipe" data-tablesaw-sortable data-tablesaw-sortable-switch data-tablesaw-minimap data-tablesaw-mode-switch>
                            ...
                            <tr>
                                ...
                                <th scope="col" data-tablesaw-sortable-col data-tablesaw-priority="5">
                                    {!! th_sort(trans('voucher.total_number'), 'total_number', $oResults) !!}
                                </th>
                                <th scope="col" data-tablesaw-sortable-col data-tablesaw-priority="5">
                                    {!! th_sort(trans('voucher.used_number'), 'used_number', $oResults) !!}
                                </th>
                            ...
                            @if (count($oResults))
                                ...
                                @foreach($oResults as $oResult)
                                    ...
                                    <tr class='{{ $class }}'>
                                                                                ...
                                                                                <td>{{ array_key_exists($oResult->type,config('array.voucher_type'))? config('array.voucher_type')[$oResult->type]:'' }}</td>

{{--                                                                                <td>{{ number_format( $oResult->amount,3) }}</td>--}}

                                                                                <td>{{ number_format( $oResult->current_amount,3) }}</td>
                                        <td>{{ $oResult->total_number }}</td>
                                        <td>{{ $oResult->used_number }}</td>

                                        <td>{{ array_key_exists($oResult->status,config('array.voucher_status'))? config('array.voucher_status')[$oResult->status]:'' }}</td>
...
@endif
                                                        @if($canEdit )
                                            <a href="/admin/voucher/{{ $oResult->id }}/edit"
                                               class="fa fa-edit"></a>
@endif
                                                            @if(false && $canShow)
                                                    <a href="/admin/voucher/{{ $oResult->id }}"
                                                       class="fa fa-file-text"></a>
@endif
                                                    <a href="{{ route('admin.voucher_users.index').'?voucher_id='.$oResult->id }}"
                                                       class="fa fa-users"></a>
